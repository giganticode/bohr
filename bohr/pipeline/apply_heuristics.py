...
def apply_lfs_to_train_set(
    lfs: List, save_generated_to: Path, save_metrics_to: Path
) -> Dict[str, Any]:
...
def apply_lfs_to_test_set(
    lfs: List, test_set: str, save_generated_to: Path, save_metrics_to: Path
) -> Dict[str, float]:
    applier = PandasLFApplier(lfs=lfs)
    ...
    return {
        f"coverage_{test_set}": coverage,
        f"majority_accuracy_{test_set}": majority_accuracy,
    }
...
def apply_heuristics(task: str) -> None:
    ...
    stats = apply_lfs_to_train_set(
        lfs, save_generated_to=task_dir_generated, save_metrics_to=task_dir_metrics
    )
