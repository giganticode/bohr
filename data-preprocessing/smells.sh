...
head -1 "$INPUT_CSV_FILE" | awk 'NF{print $0 ";smelly"}' >> "$SMELL_TEST_FILE"
cat "$INPUT_CSV_FILE" | grep -e "\(\d\+\)\{3\};long method;\(major\|critical\)" | tail -n "$n_positive_test" | awk 'NF{print $0 ";1"}' >> "$SMELL_TEST_FILE"
